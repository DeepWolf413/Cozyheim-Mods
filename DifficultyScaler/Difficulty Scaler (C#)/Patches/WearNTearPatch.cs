...
{
    [HarmonyPatch("RPC_Damage"), HarmonyPrefix]
    private static void WearNTear_RPC_Damage_Prefix(ref HitData hit)
    {
        ...
        if (!attacker.IsMonsterFaction(Time.time) || !hit.GetAttacker().IsBoss()) {
            return;
        }

        float multiplier = 1f;
        if (Main.TryGetMonsterDamage(attacker.name, out float value)) {
        ...
        if (attacker.TryGetComponent(out DifficultyScalerBase difficultyScalerBase)) {
            ...
            foreach (DifficultyScalerMultiplier multiplierType in multiplierTypes) {
                ...
                ConsoleLog.Print($"{attacker.name} -> {multiplierType.ToString()}: {difficultyScalerBase.GetMultiplier(multiplierType)}", LogType.Info);
            }
