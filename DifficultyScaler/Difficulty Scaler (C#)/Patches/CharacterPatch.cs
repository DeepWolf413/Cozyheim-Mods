...
internal static class CharacterPatch
{
	...
	private static void Setup(Character __instance)
	{
		...
		if (!__instance.TryGetComponent<ZNetView>(out var nview) || !nview.IsOwner()) return;

		if (nview.GetZDO() == null) return;

		if (Main.TryGetMonsterBaseHealthOverride(__instance.name, out var baseHealthOverrideValue)) __instance.SetMaxHealth(baseHealthOverrideValue);

		if (!__instance.TryGetComponent(out DifficultyScalerBase difficultyScalerComponent)) difficultyScalerComponent = __instance.gameObject.AddComponent<DifficultyScalerBase>();
		...
		foreach (var multiplier in multipliers) {
			if (!Main.IsMultiplierEnabled(multiplier.Key)) continue;
	...
	private static void Character_ApplyDamage_Prefix(Character __instance, ref HitData hit)
	{
		if (hit.HaveAttacker())
			if (__instance.m_faction == Character.Faction.Players) {
				var startDamage = hit.GetTotalDamage();
				var multiplier = 1f;

				if (Main.TryGetMonsterDamage(hit.GetAttacker().name, out var value)) {
					var oldBaseDamage = hit.GetTotalDamage();
					...
					ConsoleLog.Print($"-> Base damage: {oldBaseDamage} -> {hit.GetTotalDamage()}");
				}
				...
				foreach (DifficultyScalerMultiplier multiplierType in multiplierTypes) {
					...
					ConsoleLog.Print($"{__instance.name} -> (ApplyDamage) {multiplierType.ToString()} Bonus: +{difficultyScalerComponent.GetMultiplier(multiplierType) * 100f:N0}%");
				}

				multiplier += difficultyScalerComponent.GetSumOfMultipliers(enabledMultipliers);
				hit.ApplyModifier(multiplier);

				ConsoleLog.Print(__instance.name + " -> (ApplyDamage) Total bonus: +" + ((multiplier - 1f) * 100f).ToString("N0") + "%");
				ConsoleLog.Print(__instance.name + " -> (ApplyDamage) Damage: " + startDamage + " -> " + hit.GetTotalDamage());
			}
	}
}